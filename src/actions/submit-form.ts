...
import { formatPhoneForMessaging, formatTo12Hour } from '@/lib/form-utils';
...
export async function submitForm(data: SubmitFormAction, form: 'contact' | 'booking') {
		...
		await sendWhatsAppMessage(
			{
				...
				phone: formatPhoneForMessaging(phone),
				service: service || '',
				date: date ? format(date, "EEEE, d 'de' MMMM 'de' yyyy", { locale: es }) : '',
